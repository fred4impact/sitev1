...
def convert_forward(apps, schema_editor):
    ...
    for pk, raw in Venue.objects.exclude(description="").values_list("id", "description"):
        Venue.objects.filter(pk=pk).update(description=quill_json_to_html(raw))
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("venues", "0002_venue_description_to_quill"),
    ]
